                ...
                @foreach ($sliders as $slider)
                    <div class="item-slide">
                        ...
                        <div class="slide-info slide-1">
                            <h2 class="f-title">Kid Smart <b>{{ $slider->title }}</b></h2>
                    ...
                    @foreach ($saleProducts as $saleProduct)
                        <div class="product product-style-2 equal-elem ">
                            <div class="product-thumnail">
                                <a href="{{ route('product.details',['slug'=>$saleProduct->slug]) }}" title="{{ $saleProduct->name }}">
                                    <figure><img src="{{ asset('assets/images/products') }}/{{ $saleProduct->image }}" width="800" height="800" alt="{{ $saleProduct->name }}"></figure>
                                </a>
                            ...
                            <div class="product-info">
                                <a href="{{ route('product.details',['slug'=>$saleProduct->slug]) }}" class="product-name"><span>{{ $saleProduct->name }}</span></a>
                                @if ($saleProduct->sale_price > 0)
                                    <div class="wrap-price">
                                        <span class="product-price">${{ $saleProduct->sale_price }}</span>
                                        <del><span class="product-price regprice">${{ $saleProduct->regu_price }}</span></del>
                                    </div>
                                @else
                                    <div class="wrap-price"><span class="product-price">${{ $saleProduct->regu_price }}</span></div>
                                @endif
                            </div>
                        </div>
                    @endforeach
                                ...
                                @foreach ($latestProducts as $latestProduct)
                                    <div class="product product-style-2 equal-elem ">
                                        ...
                                        <div class="product-info">
                                            ...
                                            @if ($latestProduct->sale_price > 0 && $sale->status == 1 && $sale->sale_date > Carbon\Carbon::now() )
                                                <div class="wrap-price">
                                                    <span class="product-price">${{ $latestProduct->sale_price }}</span>
                                                    <del><span class="product-price regprice">${{ $latestProduct->regu_price }}</span></del>
                                                </div>
                                            @else
                                                <div class="wrap-price"><span class="product-price">${{ $latestProduct->regu_price }}</span></div>
                                            @endif
                                        </div>
                                    </div>
                                @endforeach
                                        ...
                                        <div class="product product-style-2 equal-elem ">
                                            ...
                                            <div class="product-info">
                                                ...
                                                @if ($c_product->sale_price > 0 && $sale->status == 1 && $sale->sale_date > Carbon\Carbon::now() )
                                                    <div class="wrap-price">
                                                        <span class="product-price">${{ $c_product->sale_price }}</span>
                                                        <del><span class="product-price regprice">${{ $c_product->regu_price }}</span></del>
                                                    </div>
                                                @else
                                                    <div class="wrap-price"><span class="product-price">${{ $c_product->regu_price }}</span></div>
                                                @endif
                                            </div>
                                        </div>
                                    @endforeach
